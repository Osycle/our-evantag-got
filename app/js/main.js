...
(function() {
	...
	var x = 0;
	var y = 0;
	$.fn.textCool = function(){
		console.log(this);
		var height = this.height();
		var child = this.find("span");
		var textHeight = child.height();
		//if(height < textHeight){
			// var ps = (( textHeight - height)/height);
			// var childFs = this.css("font-size").match(/\d+/gim)[0]*1;
			// var x = (childFs*ps);
			// console.log( x, ps,childFs, height, textHeight, (childFs*(ps*ps)) );
			// child.css("font-size", (x+"px") );
		//}
		...
		y++;
		if(height < textHeight)
			child.css("font-size", ((childFs-x)+"px") )	
		else if(height > textHeight)
			child.css("font-size", ((childFs+y)+"px") )	
		...
		function changeDesc(currentSlide){
			...
			$(".name").text(username).hide().fadeIn(800);
			$(".person-img img").attr("src", avatar).hide().fadeIn(800);
			console.log(avatar);
			$(".person-img .img").css("background-image", "url('"+avatar+"')").hide().fadeIn(800);
		...
		function stepSlide(currentSlide){
			...
			// $(".textfill").textfill({
			// 	innerTag: "",
		 //    //maxFontPixels: 71231234
		 //    minFontPixels: 6124
			// });
			$(".person-name").textCool();
